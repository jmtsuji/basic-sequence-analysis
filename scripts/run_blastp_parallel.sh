...
if [[ ! -f "${query_filepath}" ]]; then
  echo "[ $(date -u) ]: ERROR: input file not found at '${query_filepath}'. Exiting..."
  exit 1
elif [[ "${query_filepath##*.}" != "faa" ]]; then
  echo "[ $(date -u) ]: WARNING: extension of input file is '${query_filepath##*.}' (expecteding 'faa')"
fi

# Check outputs
if [[ -f "${output_filepath}" ]]; then
  echo "[ $(date -u) ]: ERROR: another file is already present at desired output filepath '${output_filepath}'. Refusing to continue..."
  exit 1
elif [[ ! -d "${output_filepath%/*}" ]]; then
  echo "[ $(date -u) ]: ERROR: desired directory for output file '${output_filepath}' does not exist. Please make this folder. Exiting..."
  exit 1
elif [[ -d "${output_tmpdir}" ]]; then
  echo "[ $(date -u) ]: ERROR: tmp output directory '${output_tmpdir}' already exists. Refusing to continue..."
  exit 1
fi

# Startup messages
echo "[ $(date -u) ]: Running ${SCRIPT_NAME}"
echo "[ $(date -u) ]: Version: ${SCRIPT_VERSION}"
echo "[ $(date -u) ]: Command run: ${SCRIPT_NAME} ${@}"
echo "[ $(date -u) ]: ######## PRESETS ########"
echo "[ $(date -u) ]: DB_NAME: '${DB_NAME}'"
echo "[ $(date -u) ]: BLASTDB: '${BLASTDB}'"
echo "[ $(date -u) ]: TAXDUMP_LOCATION: '${TAXDUMP_LOCATION}'"
echo "[ $(date -u) ]: LINEAGE_REPO_LOCATION: '${LINEAGE_REPO_LOCATION}'"
echo "[ $(date -u) ]: ##### USER SETTINGS #####"
echo "[ $(date -u) ]: query_filepath: '${query_filepath}'"
echo "[ $(date -u) ]: output_filepath: '${output_filepath}'"
echo "[ $(date -u) ]: evalue: '${evalue}'"
echo "[ $(date -u) ]: max_target_seqs: '${max_target_seqs}'"
echo "[ $(date -u) ]: threads: ${threads}"
echo "[ $(date -u) ]: #########################"
...
echo "[ $(date -u) ]: Running BLAST on ${threads} threads (will take time)"
cat "${query_filepath}" | \
...
# Add header
echo "[ $(date -u) ]: Performing intermediate file prep"
...
echo "[ $(date -u) ]: Getting NCBI linages (don't be surprised if a small number of errors are printed below; usually a few taxid's don't match the taxdump)"
...
echo "[ $(date -u) ]: Checking and combining outputs"
...
if [ ${output_status} != 0 ]; then
  echo "[ $(date -u) ]: ERROR: lineages table does not match the order of the original BLAST results. Cannot continue. Exiting..."
  exit 1
fi
...
echo "[ $(date -u) ]: ${SCRIPT_NAME}: finished."
